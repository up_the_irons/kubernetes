...
func (e *streamProtocolV2) createStreams(conn streamCreator) error {
	var err error
	headers := http.Header{}
...
func (e *streamProtocolV2) setupErrorStreamReading() chan error {
...
func (e *streamProtocolV2) copyStdin() {
	if e.Stdin != nil {
		var once sync.Once
		...
		go func() {
			...
			defer once.Do(func() { e.remoteStdin.Close() })

			if _, err := io.Copy(e.remoteStdin, e.Stdin); err != nil {
		...
		go func() {
			...
			if _, err := io.Copy(ioutil.Discard, e.remoteStdin); err != nil {
				runtime.HandleError(err)
			}
		}()
	}
...
func (e *streamProtocolV2) copyStdout(wg *sync.WaitGroup) {
	if e.Stdout != nil {
		wg.Add(1)
		go func() {
			defer runtime.HandleCrash()
			...
			if _, err := io.Copy(e.Stdout, e.remoteStdout); err != nil {
				runtime.HandleError(err)
			}
		}()
	}
...
func (e *streamProtocolV2) copyStderr(wg *sync.WaitGroup) {
	if e.Stderr != nil && !e.Tty {
		wg.Add(1)
		go func() {
			defer runtime.HandleCrash()
			...
			if _, err := io.Copy(e.Stderr, e.remoteStderr); err != nil {
				runtime.HandleError(err)
			}
		}()
	}
...
func (e *streamProtocolV2) stream(conn streamCreator) error {
